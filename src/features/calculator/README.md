...
The pure product pipeline, production calculator journey and results handoff are implemented. See [Milestone 3 architecture](../../../docs/product/milestone-3-architecture.md) for contracts, state/privacy decisions, validation and remaining form wiring.

Start at `/calculator`. The shared calculator layout keeps one canonical draft and latest result in memory across all seven input steps, review and `/calculator/results`. Refresh clears them. See [journey details](../../../docs/product/calculator-journey.md).
