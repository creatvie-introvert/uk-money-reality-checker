...
for (const width of [1440, 1024, 768, 390, 320]) test(`privacy, accessibility and footer at ${width}px`, async ({ page }) => {
  ...
  for (const [route, title] of [["/privacy", "Privacy"], ["/accessibility", "Accessibility"]]) {
    ...
    for (const label of ["Privacy", "Accessibility"]) await expect(page.getByRole("navigation", { name: "Legal & support" }).getByRole("link", { name: label, exact: true })).toHaveAttribute("href", `/${label.toLowerCase()}`);
  ...
  await page.getByRole("navigation", { name: "Legal & support" }).getByRole("link", { name: "Privacy", exact: true }).click();
