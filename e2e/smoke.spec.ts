...
test("public homepage opens the existing calculator", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1 })).toHaveText("See what a move could mean for your monthly money.");
  await page.locator("#hero").getByRole("link", { name: "Compare your move", exact: true }).click();
