...
const { buildCouncilTaxRelease } = await import("../../src/data/ingestion/council-tax/release.ts");
const result = buildCouncilTaxRelease();
if (result.status !== "SUCCESS") {
  console.error(JSON.stringify(result, null, 2));
  process.exitCode = 1;
} else {
  const directory = new URL("../../src/data/generated/2026-27-v1/council-tax/", import.meta.url);
  const files = {
  ...
  console.log(`${check ? "Verified" : "Generated"} 3 deterministic council-tax files: 56 charges; London city-default coverage remains unresolved.`);
}
