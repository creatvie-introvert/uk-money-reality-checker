...
const { buildWaterRelease } = await import("../../src/data/ingestion/water/release.ts");
...
if (result.status !== "SUCCESS") {
  ...
  const target = new URL("../../src/data/generated/2026-27-v1/water/", import.meta.url);
  const files = {
  ...
  for (const [name, value] of Object.entries(files)) {
    ...
    if (check) {
      if (await readFile(url, "utf8") !== bytes) throw new Error(`Generated artifact differs: water/${name}`);
    } else await writeFile(url, bytes);
  }
  console.log(`${check ? "Verified" : "Generated"} water: ${result.audit.records.length} records, 3 deterministic artifacts.`);
}
