...
export function ResultsHeader({ onRestart }: { onRestart?: () => void } = {}) {
  return <header className={styles.header}><Link href="/" className={styles.brand}>UK Money Reality<small>REAL NUMBERS. BRIGHTER DECISIONS.</small></Link>
    <nav aria-label="Main navigation"><Link href="/calculator">Move Calculator</Link><span aria-disabled="true" title="City pages are not connected yet">Cities</span><a href="#breakdown">Compare</a><a href="#methodology">How it works</a>{onRestart ? <button type="button" className={styles.outlineButton} onClick={onRestart}>New comparison</button> : <Link className={styles.outlineButton} href="/calculator">New comparison</Link>}</nav>
  </header>;
}
...
export function ResultsPage({ model, onRestart, onEdit }: { model: ResultsViewModel; onRestart?: () => void; onEdit?: (category?: string) => void }) {
  return <div className={styles.page}><a className={styles.skip} href="#results-main">Skip to results</a><ResultsHeader onRestart={onRestart} />
...
export function EmptyResults() {
  return <div className={styles.page}><ResultsHeader /><main className={styles.container}><section className={styles.hero}><div><span className={styles.eyebrow}>Your move reality</span><h1>Your comparison starts with your inputs</h1><p>Start the calculator to see your result. Inputs and results stay in memory only; refreshing or leaving the calculator clears them.</p><Link className={styles.outlineButton} href="/calculator">Start calculator</Link></div></section></main></div>;
}
