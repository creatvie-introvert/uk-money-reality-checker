"""Replay the manually reviewed official fare cells; never discover new fares.
...
ledger = json.loads((root / 'scripts/data/transport-reviewed-sources.json').read_text())
outputs = []
for source in ledger['sources']:
    if args.capture_dir:
        captures = [source] + source.get('supportingEvidence', [])
    ...
    outputs.append((root / f"src/data/controlled/transport/{source['key']}-2026-09.json", json.dumps(extract, indent=2) + '\n'))
...
print(f"{'Verified' if args.check else 'Extracted'} {sum(len(s['rows']) for s in ledger['sources'])} reviewed fare cells in {len(outputs)} source extracts.")
